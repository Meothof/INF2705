...
float facteurDeform = 1.0;       // facteur de déformation de la surface
glm::vec4 bDim = glm::vec4( 2.0, 2.0, 2.0, 1.0 );  // les dimensions de la boite en x,y,z

float temps = 0.0;               // le temps courant (en secondes)
...
void calculerPhysique( )
{
   if ( enmouvement )
   {
      ...
      if ( fmod( temps, 10 ) <= dt )
      {
         ...
         if ( indiceFonction ) { if ( ++indiceFonction > 6 ) indiceFonction = 1; chargerNuanceurs(); }
         if ( indiceTexture ) { if ( ++indiceTexture > 9 ) indiceTexture = 1; indiceDiffuse = indiceTexture; chargerNuanceurs(); }
      ...
      static int sensZ = +1;
      facteurDeform += 0.01 * sensZ;
      if ( facteurDeform < 0.1 ) sensZ = +1.0;
      else if ( facteurDeform > 1.0 ) sensZ = -1.0;
#endif
      ...
      // if ( phiCam < MPHI || phiCam > 180.0-MPHI ) sensPhi = -sensPhi;
      phiCam = 75.0 + 20.0 * sin(temps); // variation lisse
      verifierAngles();
#endif
...
void FenetreTP::afficherScene()
{
   ...
   glUniform1f( locfacteurDeform, facteurDeform );
...
void FenetreTP::clavier( TP_touche touche )
{
   ...
   case TP_MOINS: // Moduler l'effet du déplacement
      facteurDeform -= 0.01;
      std::cout << " facteurDeform=" << facteurDeform << std::endl;
      break;
