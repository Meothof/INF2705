...
int texnumero = 1;             // numéro de la texture utilisée: 0-aucune, 1-étincelle, 2-oiseau, 3-bonhomme
...
float tempsMax = 5.0;            // temps de vie maximal (en secondes)
...
void calculerPhysique( )
{
   if ( enmouvement )
   {
        ...
        glDrawArrays( GL_POINTS, 0, sizeof(part)/sizeof(Part));
...
void chargerTextures()
{
   ...
   {
      glGenTextures( 1, &textureBONHOMME );
      glBindTexture( GL_TEXTURE_2D, textureBONHOMME );
      glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, largeur, hauteur, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels );
      glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
      glBindTexture( GL_TEXTURE_2D, 0 );
      delete[] pixels;
...
void FenetreTP::afficherScene()
{
   ...
   matrVisu.LookAt( ptVise.x + distCam*cos(glm::radians(thetaCam))*sin(glm::radians(phiCam)),
                    ptVise.y + distCam*sin(glm::radians(thetaCam))*sin(glm::radians(phiCam)),
                    ptVise.z + distCam*cos(glm::radians(phiCam)),
   ...
   case 3: glBindTexture( GL_TEXTURE_2D, textureBONHOMME ); break;
   }
...
void FenetreTP::clavier( TP_touche touche )
{
   ...
   case TP_l: // Incrémenter la durée de vie maximale
      tempsMax += 0.2;
      std::cout << " tempsMax=" << tempsMax << std::endl;
      break;
   case TP_k: // Décrémenter la durée de vie maximale
      tempsMax -= 0.2;
      if ( tempsMax < 1.0 ) tempsMax = 1.0;
      std::cout << " tempsMax=" << tempsMax << std::endl;
      break;
...
void FenetreTP::sourisMouvement( int x, int y )
{
   if ( pressed )
   {
      ...
      case deplaceCam:
         ...
         break;
      case deplacePuits:
         positionPuits.x += 0.01 * dx;
         positionPuits.y -= 0.01 * dy;
         verifierPositionPuits();
         break;
      }
